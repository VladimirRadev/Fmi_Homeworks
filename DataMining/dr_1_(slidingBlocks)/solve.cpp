...
int main()
{
    ...
    else
    {
        ...
        // std::cout << "Manhatton of start state: " << h(initialMatrix) << std::endl;

        Node root(initialMatrix, 0, h(initialMatrix), std::vector<std::string>(), blankIndexes);
        ida_star(&root);
        // for (size_t i = 0; i < gsIndexes.size(); i++)
        // {
        //     std::cout<<"Num:" <<i<< " : " <<gsIndexes[i].first<<' '<<gsIndexes[i].second<<std::endl;
        // }
