...
void initCentroidsPlusPlus(int K)
{
	// choose first point
	int pointIndex = rand() % points.size();
	indexesOfPointsCentroids.insert(pointIndex);

	pointDistances.resize(points.size(), 0.0);
	...
	while (counter < K - 1)
	{
		...
		{
			//if the point is not centroid
			if (indexesOfPointsCentroids.find(i) == indexesOfPointsCentroids.end())
			{
		...
		while (indexesOfPointsCentroids.find(newCentroidIndex) != indexesOfPointsCentroids.end())
		{
			newCentroidIndex = chooseNewCentroid();
		}

		indexesOfPointsCentroids.insert(newCentroidIndex);
		counter ++;
	}

	for (int pointIndex : indexesOfPointsCentroids)
	{
		Point* centroid = new Point(points[pointIndex]->x, points[pointIndex]->y);
		centroids.push_back(centroid);
	}
}
